...
export default function EveryMemoryMain() {
  ...
  useEffect(() => {
    if (posts.length > 0) {
      ...
      if (allTag.length > 0) {
        //set은 생성자 함수, 하지만 일반 함수처럼 호출 불가. 따라서 new랑 짝궁=>set 객체 만들어짐=>[...new~]=>배열열
  ...
  const weeklyPostIds = useMemo(
    () => new Set(weeklyPosts.map((weeklyPost) => weeklyPost.post_id)),
    [weeklyPosts]
  );
  const dailyPosts = useMemo(
    () => posts.filter((post) => !weeklyPostIds.has(post.post_id)),
    [posts, weeklyPostIds]
  ); //has는 Set,Map에 사용하는 include,some보다 빠르게 작동함.
    ...
    <div>
      <div className={styles.mainContainer}>
        ...
        <div className={styles.forFlexTagBox}>
          <div className={styles.tagBox}>
            <img
              src={landscape}
              alt=""
              className={styles.tagBoxLandscape}
            ></img>
        ...
        <div className={styles.todayTagAllContainer}>
          <div
            className={styles.forModalContainer}
            onClick={() => {
        ...
        <div className={styles.forFlexButton}>
          ...
          <img
            alt=""
            src={num1}
            className={styles.num1Icon}
            onClick={() => onClickNextPage(0)}
          />
          <img
            alt=""
            src={num2}
            className={styles.num2Icon}
            onClick={() => onClickNextPage(1)}
          />
          <img
            alt=""
            src={num3}
            className={styles.num3Icon}
            onClick={() => onClickNextPage(2)}
          />
          <img
            alt=""
            src={num4}
            className={styles.num4Icon}
        ...
        <div className={styles.postImageContainerInner}>
          ...
          <div
            className={styles.postImageToolContainer}
            onClick={handleContainerClick}
          >
            <img src={cloud} alt="" className={styles.cloudIcon}></img>
            <p className={styles.postImageToolText}>
              이곳을 클릭하거나 사진을 드래그하여 업로드하세요.
            </p>
            <p className={styles.ImageInfo}>
              지원 형식: JPG, PNG, HEIC / 최대 파일 크기: 5MB
            </p>
            <input
              type="file"
              ref={fileInputRef}
