...
export default function DailyPopularTag({post, handleLikeClick, handleCommentClick}) {
    ...
    return(
        <div className={styles.todayTagContainer}>
            <span className={styles.todayTagImage}
            style={{backgroundImage:`url(${post?.photo_url || {emptyImage}})`}}></span>
            <div className={styles.forFlexTodayTag1}>
                <div className={styles.forFlexTodayTag2}>
                    {/*여기 아이콘은 빼야 할 듯*/}
                    <span className={styles.todayTagImageName}>{post.post_text}{/*앨범 제목 받아오기*/}</span>
                </div>
